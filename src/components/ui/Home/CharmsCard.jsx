...
export const CharmsCard = ({ name, imgSrc, category }) => {

    return (
        <Link to="/catalog/charms" className="group w-full">
            <div className="relative flex flex-col items-center gap-3 rounded-2xl border border-neutral-800 bg-neutral-900/80 px-5 py-6 shadow-md transition hover:-translate-y-1 hover:border-white/50">
                <span className="absolute right-4 top-3 text-[0.7rem] uppercase tracking-[0.2em] text-gray-400">{category}</span>
                <div className="w-24 h-24 flex items-center justify-center rounded-full bg-gradient-to-b from-neutral-800 to-black">
                    <ImgLoadingPlaceholder src={imgSrc} alt={name} className="h-16 object-contain transition-transform duration-300 group-hover:scale-105" />
                </div>
